...
prop_iter :: operator++()
{
    while (internal_cur != internal_end) {
        ...
        if (internal_cur != internal_end
         && order::clock_creat_before_del_after(req_time, internal_cur->get_creat_time(), internal_cur->get_del_time())) {
            break;
        }
    }
    return *this;
...
prop_iter :: prop_iter(prop_vec_t::iterator begin, prop_vec_t::iterator end,
    vc::vclock &req_time)
    : internal_cur(begin)
    , internal_end(end)
    , req_time(req_time)
