            ...
            vc::vclock &req_vclock)
    {
        ...
        if (!params.returning) { // request mode
            if (params.dest == rn.handle) {
                ...
                false_reply = params.hops == max_hops-1;
                // have not found it yet or previously found it with longer path and don't not maxed out hops yet
                if (!false_reply && ((state.already_visited_hops > 0 && parms.max_hops == MAX_UINT64) || (state.already_visited_hops > params.hops))) {
                    db::element::edge *e;
                    state.prev_node = prev_node;
                    state.already_visited_hops = parms.hops;
