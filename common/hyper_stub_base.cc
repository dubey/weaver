    ...
    std::vector<hyperdex_client_map_attribute*> &map_attrs, std::vector<size_t> &map_num_attrs)
{
    ...
    uint64_t i = 0;
    for (; i < num_calls; i++) {
        HYPERDEX_CALL7(funcs[i], hyper_tx,
                       spaces[i], keys[i], key_szs[i],
                       attrs[i], num_attrs[i],
                       &call_status[i],
                       hdex_id, call_status[i], success, success_calls);
    ...
    for (uint64_t j = 0; j < map_num_calls; j++, i++) {
        HYPERDEX_CALL7(map_funcs[i], hyper_tx,
    ...
    bool tx)
{
    ...
    if (tx) {
        HYPERDEX_CALL7(hyperdex_client_xact_get, hyper_tx,
                       space, key, key_sz,
                       &get_status,
                       cl_attr, num_attrs,
    ...
    bool tx)
{
    ...
    if (tx) {
        HYPERDEX_CALL9(hyperdex_client_xact_get_partial, hyper_tx,
                       space, key, key_sz,
        ...
        HYPERDEX_CALL9(hyperdex_client_get_partial, cl,
                       space, key, key_sz,
                       attrnames, attrnames_sz,
                       &get_status,
                       cl_attr, num_attrs,
...
hyper_stub_base :: del(const char* space, const char *key, size_t key_sz)
{
    ...
    HYPERDEX_CALL5(hyperdex_client_xact_del, hyper_tx,
                   space, key, key_sz,
                   &del_status,
                                 ...
                                 std::vector<db::edge*> &edge_vec)
{
    ...
    unpack_buffer(cl_attr[idx[1]].value, cl_attr[idx[1]].value_sz, edge_vec);
    ...
    if (success) {
        for (i = 0; i < num_edges; i++) {
            if (*num_attrs[i] == NUM_EDGE_ATTRS) {
                edge_handle_t handle;
                std::vector<db::edge*> edge_vec;
                success = recreate_edge(attr_array[i], handle, edge_vec) && success;
                n.out_edges[handle] = edge_vec;
                                ...
                                db::edge *e)
{
    ...
    size_t packed_sz;
    prepare_edge(attrs, *e, edge_buf, packed_sz);
