...
{
    return replicant_client_call(m_repl, "weaver", func, data, data_sz, REPLICANT_CALL_ROBUST, status, output, output_sz);
}
...
server_manager_link :: wait(const char* cond, uint64_t state,
                         ...
                         char **output, size_t *output_sz)
{
    return replicant_client_cond_wait(m_repl, "weaver", cond, state, status, output, output_sz);
}
...
server_manager_link :: begin_waiting_on_broadcast(replicant_returncode* status)
{
    ...
    m_id = replicant_client_cond_wait(m_repl,
                                      "weaver",
                                      "config",
                                      m_config.version() + 1,
                                      &m_status,
                                      &m_output, &m_output_sz);
    m_state = WAITING_ON_BROADCAST;
    *status = m_status;
...
server_manager_link :: begin_fetching_config(replicant_returncode* status)
{
    ...
    assert(m_output == NULL);
    assert(m_output_sz == 0);
    m_id = replicant_client_call(m_repl, "weaver", "config_get", "", 0,
                                 REPLICANT_CALL_ROBUST, &m_status, &m_output, &m_output_sz);
    m_state = FETCHING_CONFIG;
    *status = m_status;
