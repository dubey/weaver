...
namespace common
{
    ...
    {
        std::pair<const Key, db::element::edge*>::iterator cur;
        std::pair<const Key, db::element::edge*>::iterator end;
        vc::vclock& req_time;

        public:
        edge_iter(std::unordered_map<uint64_t, edge*>& edges, vc::vclock& req_time) : cur(edges.begin()), end(edges.end()), req_time(req_time) {}
