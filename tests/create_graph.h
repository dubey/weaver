...
create_elems(client::client *c)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        WDEBUG << "Created node, handle = " << nodes[i] << std::endl;
        c->end_tx(tx_id);
    }
    // edges
    uint64_t tx_id = c->begin_tx();
    ...
    for (i = 0; i < 10; i+=2) {
        ...
        WDEBUG << "Created node, handle = " << nodes[i+1] << std::endl;
        c->end_tx(tx_id);
    }
}
...
create_graph_test()
{
    int i;
    if (NUM_VTS == 1) {
        WDEBUG << "single vts" << std::endl;
        client::client c(CLIENT_ID, 0);
        create_elems(&c);
    } else {
        // multiple clients
        WDEBUG << "multiple vts, multiple clients" << std::endl;
        client::client* cl[50];
        for (i = 0; i < 50; i++) {
