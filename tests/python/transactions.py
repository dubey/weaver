...
def create_edges(c, n_edges):
    ...
    with gmutex:
        while n_edges > 0:
            n1 = get_random_idx()
            n2 = get_random_idx()
            while n2 == n1 or check_repeat(n1, n2, edges):
                n2 = get_random_idx()
...
def reach_requests(c, n_reqs):
    ...
    with gmutex:
        for i in range(n_reqs):
                ...
                while n2 == n1:
                    n2 = get_random_idx(n1 % 2)
                pos_reqs.append((n_hndls[n1], n_hndls[n2]))
                n1 = get_random_idx()
                n2 = get_random_idx(1 if n1 % 2 == 0 else 0)
                neg_reqs.append((n_hndls[n1], n_hndls[n2]))
    ...
    with gmutex:
        ...
        for i in range(int(edge_factor * num_nodes)):
            ...
            while n2 == n1: # no self-loops
                n2 = get_random_idx(n1 % 2)
            if n1 >= len(n_hndls):
                print 'bad idx: ' + str(n1)
                pdb.set_trace()
            if n2 >= len(n_hndls):
                print 'bad idx: ' + str(n2)
                pdb.set_trace()
...
def write_loop(c, tid):
    for i in range(wr_loops):
        assert edge_cnt == int(num_nodes * edge_factor)
        create_edges(c, 100)
        delete_edges(c, 100)
        with pmutex:
            print str(tid) + ': Write loop ' + str(i)
